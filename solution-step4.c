...
void updateBody() {
    ...
    double min_tStep = 1e-4;

    // If minDx < 10^-9 -> Set timeStep to min
    if(minDx < 1e-9){
      timeStepSize = min_tStep;

    } else if(2 * maxV * timeStepSize >= minDx){
      // If Particles might collide in next step, halve timeStep
      while(2 * maxV * timeStepSize >= minDx){
        timeStepSize /= 8;
      }
      // Cap timeStep at min_tStep
      if(timeStepSize < min_tStep){
        timeStepSize = min_tStep;
      }

    } else {
      // If no conditions above are satisfied, we can increase timeStep by 1%
      timeStepSize *= 1.01;
    }
